...
class Caregiver:
    ...
    def get(self):
        ...
        get_caregiver_details = "SELECT Salt, Hash FROM Caregivers WHERE UsernameC = %s"
        try:
    ...
    #     search_care_schedule = "SELECT UsernameC as username FROM Availabilities as a WHERE a.Time = %s"
    #     try:
    #         cursor.execute(search_care_schedule, formatted_date)
    #         # you must call commit() to persist your data if you don't set autocommit to True
    #         conn.commit()
    #         for row in cursor:
